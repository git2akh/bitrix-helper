...
GIT_REPO_LATEST_DATA="$(curl -L --max-redirs 10 ${GIT_REPO_API_BASE}/releases/latest)";
...
GIT_LATEST_FETCH_zipball_CMD="curl -L --max-redirs 10 ${GIT_LATEST_zipball_url} | tee \"${GIT_AUTHOR}-${GIT_REPO}-${GIT_REPO_LATEST_TAG}.zip\" | wc -c";
GIT_LATEST_FETCH_tarball_CMD="curl -L --max-redirs 10 ${GIT_LATEST_tarball_url} | tee \"${GIT_AUTHOR}-${GIT_REPO}-${GIT_REPO_LATEST_TAG}.tar.gz\" | wc -c";
